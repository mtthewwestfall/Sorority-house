...
def _character_reference(char_id: str, asset_id: Optional[int], *, strict: bool = True) -> Optional[tuple]:
    ...
    Explicit reference asset is tried first.
    If no explicit asset (or explicit asset fails), tagged skins, disk uploads, or repo owner skin are used.
    strict=True (primary / webcam): an explicit asset_id that cannot be loaded raises.
    strict=False (Sogni): an explicit asset_id that cannot be loaded returns None so generation continues text-only."""
