...
async function importDocs(files){files=[...files];$('#docFiles').value='';if(!files.length)return;const known=PERS.filter(p=>!p.isNew).map(p=>p.girl);
 ...
 let ok=0,skipped=0;const saved=new Map();for(const {f,slug} of plan){try{const text=await f.text();if(!text.trim())throw new Error(f.name+' is empty');
  ...
  const cur=saved.has(slug)?saved.get(slug):(p.persona||'');const doc=mergeDoc(cur,text,saved.has(slug)?'append':mode);if(doc===null){skipped++;continue}
  await api('/admin/console/girl',{method:'POST',body:JSON.stringify({girl:slug,name:p.name,door_title:p.door_title,blurb:p.blurb,avatar_url:p.avatar_url,
   min_tier:p.min_tier,sort_order:p.sort_order,difficulty:p.difficulty||'normal',persona:doc,active:p.active})});saved.set(slug,doc);ok++}catch(e){toast(e.message,true)}}
