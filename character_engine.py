...
# OPTIMIZATION (Bolt ⚡): Pre-compiled combined regex patterns to avoid recompiling and looping over patterns on every intent evaluation
_OFF_CARD_RE = re.compile("|".join(OFF_CARD_PATTERNS), re.IGNORECASE)
_COMMAND_RE = re.compile("|".join(COMMAND_PATTERNS), re.IGNORECASE)
_DIRECT_RE = re.compile("|".join(DIRECT_PATTERNS), re.IGNORECASE)
