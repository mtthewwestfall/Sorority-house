...
| `BRAIN_MAX_TOKENS` / `AUDIT_MAX_TOKENS` | Token budgets for the memory digest (default `4000`) and the audit report (default `900`); reasoning models bill their thinking against them, so raise `AUDIT_MAX_TOKENS` when the audit runs on `deepseek-reasoner` | You choose it |
