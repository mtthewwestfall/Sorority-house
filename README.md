...
Key environment variables (Railway): `DATABASE_URL`, `ADMIN_SECRET`, `GEMINI_API_KEY`, `STRIPE_WEBHOOK_SECRET`, `SHOPIFY_WEBHOOK_SECRET`, `NEXAPAY_WEBHOOK_SECRET`, `NOWPAYMENTS_IPN_SECRET`, `NOWPAYMENTS_API_KEY`, `MAIL_FROM`, `SITE_PASSWORD`, `VIDEO_MODEL`, `SOGNI_API_KEY`, `SOGNI_IMAGE_MODEL`, `SOGNI_API_URL`, `KEYHOLE_MEDIA_DIR`, `KEYHOLE_CONTENT_SITES`, `TELEGRAM_BOT_TOKEN` (public-show DMs), `PAY_LINK_PUBLIC` ($4.99 lounge link in that announcement), `SHOPIFY_TEXT_STORE` / `TEXT_ONLY_HANDLE` / `TEXT_ONLY_SKU` / `TEXT_ONLY_VARIANT_ID` (the $5.99 Text-Only pack sold through Shopify; the orders webhook grants its texts to the buyer's account, stacking past the monthly cap — completed sales are never refused), `PAY_LINK_TEXT` (optional override for the Telegram text-pack button). Keyhole plate generators, the admin content maker, and the public-show blast are documented in the `main.py` docstring.
