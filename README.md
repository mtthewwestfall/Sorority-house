...
- **Bring a Wingman (referrals):** every account has a referral link (`GET /referral` → `SITE_URL/?ref=<code>`; the page remembers `?ref` and sends it with the signup). A referral **qualifies** when the friend later buys **Junior or Senior** — Sophomore does not count (`tier_rank(tier) >= tier_rank("junior")`) — and each friend qualifies at most once, so renewal invoices never re-trigger anything. The referrer's **first** qualifying referral is the Wingman: both the referrer and the friend are comped to Senior for 30 days through the ordinary free-time columns (`comp_prev_*` remember the exact tier, usage and reset each had a moment before), so when it runs out `_ensure_user` returns each of them to their own paid tier, never to Freshman by default. If the referrer is already Senior their tier is left alone and they get 30 more days instead — added to `comp_until` if that Senior is comped, to `plan_reset_at` if it is paid; the friend still gets the normal 30-day comp. A real Stripe tier change in the window clears the comp as usual and wins. House-wide there are at most **100 Wingman pairs**, checked under a lock so simultaneous purchases can't slip past it; **5 qualifying referrals** flag the referrer for the Wingman shirt.
...
- **Referrals:** the Overview tab shows every referrer with signups / qualified / Wingman / shirt, pairs used against the 100 cap, and the **program switch** (`GET /admin/referrals`, `POST /admin/referrals/program {on}`). Off: signups are still attributed, but no purchase qualifies, nothing is counted and no Wingman is granted until it is back on. Each account page shows who referred them and their own referral tally.
...
- **Doors:** opened by progression, not by tier. The first set in roster order (by default a pair: Dakota and Zoe) is open from the start; each next set unlocks once the player reaches the unlock stage (default M4) with any girl of the set directly before it. The Roster tab's "Doors" card sets the set size and stage, or turns locking off so every door is open (`GET`/`POST /admin/console/doors`). A girl's `min_tier` is only a paywall on top of that — by default everyone is on Freshman except Veronica (Senior).
...
| `STRIPE_API_KEY` | Restricted key (Customers: read, Subscriptions: read). Required for purchases started from the Telegram bot (the tier is read from the subscription); also matches cancellations of customers that never paid through this webhook. Add Subscriptions: write + PaymentIntents: write and the webhook also stamps the Affitor affiliate metadata (`affitor_click_id`, `affitor_customer_key`, `program_id`) on each paid subscription | Stripe Dashboard → API keys |
| `AFFITOR_PROGRAM_ID` | Affitor Wingman program id written into that metadata (default `1082`). The tracker in `web/index.html` drops the click id cookie; signup/login send it to the backend | Affitor dashboard |
