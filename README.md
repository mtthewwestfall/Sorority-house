...
Run it as a second Railway service from this repo: start command `python telegram/bot.py`, variables `TELEGRAM_BOT_TOKEN` (a new bot from @BotFather) and `PUBLIC_URL` (the backend URL). Optional: `SITE_URL` (default `https://lockeddoor.ai`, used for portraits and the `/menu` website + "Get the app" buttons) and `PAY_LINK_SOPHOMORE` / `PAY_LINK_JUNIOR` / `PAY_LINK_SENIOR` (the Stripe Payment Links behind `/upgrade`; the live ones are the defaults). Payments never happen inside Telegram — checkout is on Stripe, the account is upgraded by email and the bot sees the new tier at once. It polls Telegram, so no public port. Passwords typed into `/login` stay in that Telegram chat's history — the bot refuses them in groups, and users can delete the message afterwards.
