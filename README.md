...
Run it as a second Railway service from this repo: start command `python telegram/bot.py`, variables `TELEGRAM_BOT_TOKEN` (a new bot from @BotFather) and `PUBLIC_URL` (the backend URL). Optional: `SITE_URL` (default `https://lockeddoor.ai`, used for portraits and the `/menu` website + "Get the app" buttons) and `UPGRADE_URL` (default `SITE_URL/#plans`; point it at a Stripe Payment Link to send people straight to checkout — payments never happen inside Telegram, the webhook upgrades the account by email and the bot sees the new tier at once). It polls Telegram, so no public port. Passwords typed into `/login` stay in that Telegram chat's history — the bot refuses them in groups, and users can delete the message afterwards.
